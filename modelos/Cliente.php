...
class Cliente extends Conexion
{
    ...
    {
        // Con esato se Valida el NIT antes de guardar los datos
        if (!$this->validarNit($this->cliente_nit)) {
            echo "El NIT ingresado es inválido. Por lo tanto, No se guardarán los datos.";
            // Con esto se Detiene la ejecución del código o redirigir a otra página, según sea necesario
            exit();
        }
    ...
    {
        // Aqui con esta funcion Eliminar cualquier guión o espacio en blanco del NIT
        $cliente_nit = str_replace(['-', ' '], '', $cliente_nit);

        // Aqui se  Verificar si el NIT tiene 8 dígitos
        if (strlen($cliente_nit) !== 8) {
            return false;
        }

        //aqui se puede Realizar la validación del NIT según el algoritmo dado
        $suma = 0;
        for ($i = 0; $i < 7; $i++) {
        ...
        //Con este se  Comprueba si el residuo es igual al dígito verificador
